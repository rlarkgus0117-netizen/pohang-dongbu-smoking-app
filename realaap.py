...
# ── 🛠️ [관리자 대시보드 및 상단 제어 바] ───────────────────────────────────────
# 첫 화면(선택하기 단계) 또는 전체 학교 대시보드가 켜졌을 때만 좌측 상단에 노출되도록 명확히 제어합니다.
if st.session_state.selected_class is None or st.session_state.show_school_dashboard:
    col_admin, col_spacer, col_btn = st.columns([2.5, 1.5, 2])
    
    with col_admin:
        admin_exp = st.expander("🔐 관리자 대시보드")
        with admin_exp:
            admin_password = st.text_input("보건교사 암호를 입력하세요:", type="password", key="admin_pwd_v6")
            if admin_password == "admin":
                st.markdown("##### 🛠️ 실시간 데이터 제어판")
                st.write("학급 완료 인원 값을 직접 수정할 수 있습니다.")
                cls_to_edit = st.selectbox("학급 고르기:", list(CAPACITIES.keys()), key="admin_cls_select_v6")
                current_val = stats.get(cls_to_edit, 0)
                max_val = CAPACITIES[cls_to_edit]
                
                new_val = st.number_input(
                    f"{cls_to_edit} 인원 수정 (정원: {max_val}명)", 
                    min_value=0, max_value=max_val, value=current_val, step=1, key="admin_num_input_v6"
                )
                
                if st.button("데이터 수정 💾", key="admin_save_btn_v6"):
                    stats[cls_to_edit] = new_val
                    save_stats(stats)
                    st.success(f"{cls_to_edit} 학급 완료 인원이 {new_val}명으로 수정되었습니다.")
                    st.rerun()
                    
                st.divider()
                st.markdown("##### 🚨 전체 초기화")
                confirm_reset = st.checkbox("데이터 전체 삭제에 완전히 동의합니다.", value=False, key="admin_confirm_reset_v6")
                if st.button("🔴 전체 학급 초기화 실행", key="admin_reset_btn_v6", disabled=not confirm_reset):
                    reset_data = {k: 0 for k in CAPACITIES.keys()}
                    save_stats(reset_data)
                    st.success("전 학급 데이터가 성공적으로 리셋되었습니다.")
                    st.rerun()
            elif admin_password:
                st.error("비밀번호가 일치하지 않습니다.")
                
    with col_btn:
        if st.button("🏫 우리 학교의 건강한 폐", key="school_btn_v7", help="학교 전체의 건강 상태 보러 가기"): 
            st.session_state.show_school_dashboard = not st.session_state.show_school_dashboard
            st.rerun()

# 타이틀 카드 출력
if not st.session_state.show_school_dashboard:
    st.markdown('<div class="main-title">🍀 평생 금연을 실천하는 포항동부초</div>', unsafe_allow_html=True)
...
if st.session_state.show_school_dashboard:
    ...
    for idx, (cls_name, cap) in enumerate(CAPACITIES.items()):
        ...
        with col:
            ...
            <div style="background-color: #FAFAFA; padding: 15px; border-radius: 12px; margin-bottom: 10px; border-left: 5px solid #FF4D6D; box-shadow: 1px 1px 4px rgba(0,0,0,0.05);">
                <span style="font-size: 18px; font-weight: bold; color: #333;">{cls_name}학년 {cls_name}반 </span>
    ...
    if st.button("🔙 다시 퀴즈 맞추러 가기", key="back_to_quiz_v6"):
        st.session_state.show_school_dashboard = False
        st.rerun()
...
# ── 4학년 영역 ────────────────────────────────────────────────────────
    ...
    if step == 1:
        ...
        if q1_status == "retry":
            ...
            st.markdown('<div class="success-box">👏 정답입니다! 한 번 고장난 중독 뇌 회로는 본인의 의지로 치유되기 어렵기 때문에, 애초에 호기심을 완벽히 꺼버려야 합니다!</div>', unsafe_allow_html=True)
            if st.button("다음 문제로 가기 ➔"):
                st.session_state.quiz_step = 2
                st.rerun()
        ...
        if q2_status == "retry":
            st.markdown('<div class="retry-box">🤔 <b>다시 한번 생각해보세요!</b><br/>신체 활동과 음악은 도파민을 건강하게 분비하도록 돕습니다. 뇌를 계속 피로하게 만드는 유해 행동이 무엇인지 골라보세요!</div>', unsafe_allow_html=True)
